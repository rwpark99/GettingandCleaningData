...
meanStdData <- mergedData[, grep("subject|activity|mean__|std__{1}", colnames(mergedData), value=TRUE, ignore.case = FALSE)]
write.table(meanStdData, file = "mergedMeanStrData.txt", append = FALSE, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

# 5. Creates a second, independent tidy data set with the average of each variable for each activity and each subject. 
summaryData = fread("mergedData.txt",  sep="auto", header=TRUE)
setkey(summaryData, subject, activity)
summaryDataWrite <- summaryData[, lapply(.SD, mean()), by="subject,activity"]
write.table(summaryDataWrite, file="tidyData.txt", sep="\t", row.names=FALSE, col.names=TRUE)

#########################################################
# End of Assignment Week 1
#########################################################